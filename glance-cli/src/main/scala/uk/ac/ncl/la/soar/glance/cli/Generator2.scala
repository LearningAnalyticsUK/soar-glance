...
/**
  ...
  */
object Generator2 extends Command[GeneratorConfig, Unit] {

  def run(conf: GeneratorConfig): Either[Throwable, Unit] = {

    //TODO: Write surveys out to database using SurveyRepository
    //TODO: Refactor to MonadError or similar so that we can bind to Task
    for {
      scores <- parseScores(conf.recordsPath)
      surveys <- Survey.generate(scores, conf.elided, conf.modules, conf.common, conf.seed)
    } yield ()
  }
